...
wide_visual_y_offset = 2.2;     % m, aligns CRG Dej Wide visuals to CRG Teste

scene_data.Name               = 'CRG_Teste_F';
scene_data.CRGfile            = 'CRG_Dej_Wide_f.crg';
scene_data.Geometry.filename  = 'CRG_Dej_Wide_f.stl';
...
scene_data.Geometry.y     = wide_visual_y_offset;           % m
...
scene_data.Geometry.centerline.filename  = 'CRG_Dej_Wide_f_centerline.stl';
...
scene_data.Geometry.centerline.y     = wide_visual_y_offset;           % m
...
wide_data = load(fullfile(fileparts(which('sm_car_scenedata_crg_dej_wide_f')), 'CRG_Dej_Wide_f_dat.mat'), 'dat');
wide_dat = wide_data.dat;
...
scene_data.Geometry.finish_line.offset.xyz   = [wide_dat.rx(1) wide_dat.ry(1)+wide_visual_y_offset 0]; % m
scene_data.Geometry.finish_line.offset.yaw   = double(wide_dat.p(1));      % rad

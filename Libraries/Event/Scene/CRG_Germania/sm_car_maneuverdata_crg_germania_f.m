function maneuver_data = sm_car_maneuverdata_crg_germania_f

maneuver_type = 'CRG_Germania_F';
...
for i=1:length(Instance_List)
    ...
    mdata.(Instance).Trajectory_LoadFile.Value    = 'CRG_Germania_f_trajectory_default.mat';
    mdata.(Instance).Trajectory_LoadFile.Units    = '';
    mdata.(Instance).Trajectory_LoadFile.Comments = '';

    mdata.(Instance).xMaxLat.Value         = 1; % m

function maneuver_data = sm_car_maneuverdata_crg_dej_wide_f

maneuver_type = 'CRG_Dej_Wide_F';
...
for i=1:length(Instance_List)
    ...
    mdata.(Instance).Trajectory_LoadFile.Value    = 'CRG_Dej_Wide_f_trajectory_default.mat';
    mdata.(Instance).Trajectory_LoadFile.Units    = '';
    mdata.(Instance).Trajectory_LoadFile.Comments = '';

    mdata.(Instance).xMaxLat.Value         = 1.8; % m
    mdata.(Instance).xMaxLat.Units         = 'm'; % m
    mdata.(Instance).xMaxLat.Comments      = 'Maneuver half-track width'; % m
    ...
    mdata.(Instance).xPreview.x.Value      = [2 2.5 5]; % m

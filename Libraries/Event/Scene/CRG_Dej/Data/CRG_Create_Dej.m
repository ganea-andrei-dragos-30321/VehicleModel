function traj_coeff = CRG_Create_Dej
...
file_data = readtable("CRG_Dej_centerline.xlsx");
if width(file_data)<5
    ...
    writetable([file_data,dist],'CRG_Dej_centerline.xlsx','WriteMode','overwrite','WriteVariableNames',true);
    clear dist dist_m
end
clear file_data;
if(nargout == 0)
    ...
    road_opts.road_width = 1.5;      % Half width of the road
    ...
    sm_car_centerline_to_crg('CRG_Dej',road_opts)
end

%% Create driver trajectory
traj_coeff.blend_distance = 10;     % m
traj_coeff.diff_exp       = 1.3;    % Curvature exponent
traj_coeff.diff_smooth    = 8;     % Diff smoothing number of points
traj_coeff.curv_smooth    = 25;    % Curvature smoothing number of points
...
if(nargout == 0)
    ...
    sm_car_trajectory_calc('CRG_Dej',traj_coeff)
    
    % Create driver trajectory for road with elevation
    sm_car_trajectory_calc('CRG_Dej_f',traj_coeff)
end